...
class Fatoora:
    ...
    def invoice_date(self) -> str:
        date_time_str = datetime.fromtimestamp(float(self.temp)).strftime("%Y-%m-%dT%H:%M:%S.%f")
        self.tags[0x03] = date_time_str[:-7]+"Z"
        return date_time_str[:-7]+"Z"

    @invoice_date.setter
    @validate_arguments
    def invoice_date(self, new_value: float) -> None:
        self.temp = str(new_value)
